...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //NSString  *ScoresinGameCenter =
    
    NSString  *ScoresinGameCenter = [[NSUserDefaults standardUserDefaults] stringForKey:@"ScoresinGameCenter"];
       ////NSLog(@"320 ScoresinGameCenter='%@'",ScoresinGameCenter);
  
   if (ScoresinGameCenter == (id)[NSNull null] || ScoresinGameCenter.length == 0 ) ScoresinGameCenter = @"Unavailable";
...
- (void) reloadScoresComplete: (GKLeaderboard*) leaderBoard error: (NSError*) error;
{
    ...
    if(error == NULL)
    {
        ...
        ////NSLog(@"352 ScoresinGameCenter = s ='%@'",s);
        
        if ([s isEqualToString:@"(null)"]) {
            s = @"Unavailable";
        }
         if (s == (id)[NSNull null] || s.length == 0 ) s = @"Unavailable";
             ////NSLog(@"357 *s = %@", s);
        ...
        if([leaderBoard.scores count] >0)
        {
            ...
            [gameCenterManager mapPlayerIDtoPlayer: allTime.player.playerID];
